      ...
      <nav className="fixed top-0 left-0 w-full z-50 border-b border-white/5 bg-transparent backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-16 md:h-24 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 md:hidden">
            <button onClick={() => setSidebarOpen(true)} className="w-10 h-10 flex items-center justify-center border border-white/10 rounded-sm bg-white/5">
              <Menu size={20} />
            </button>
          </div>
          ...
          <div className="hidden md:flex items-center gap-10">
            {NAV_LINKS.map((link) => (
              <Link key={link.href} href={link.href} className="text-gray-400 hover:text-white transition-colors font-mono text-[11px] tracking-[0.3em] font-black uppercase">
                {link.label}
              </Link>
            ))}
          </div>
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-orange-500/30 selection:text-orange-200 overflow-x-hidden">
      ...
      <div className="fixed top-0 left-0 right-0 h-[3px] bg-white/5 z-[100]">
        <motion.div className="h-full bg-gradient-to-r from-orange-600 via-orange-500 to-cyan-400 origin-left" style={{ scaleX }} />
      </div>
      ...
      <section className="relative z-10 min-h-[70vh] flex flex-col items-center justify-center pt-28 pb-16 px-4">
        <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="text-center max-w-5xl mx-auto">
          <motion.div variants={fadeInUp} className="inline-flex items-center gap-3 px-5 py-2 border border-orange-500/20 bg-orange-500/5 rounded-full mb-8 backdrop-blur-sm">
            <Handshake size={14} className="text-orange-500" />
            <span className="font-mono text-[9px] text-orange-400 tracking-[0.5em] font-black uppercase">Strategic Partnership Program</span>
          </motion.div>

          <motion.h1 variants={fadeInUp} className="text-5xl sm:text-7xl md:text-[9rem] font-black leading-[0.8] tracking-tighter uppercase italic mb-6">
          ...
          <motion.p variants={fadeInUp} className="text-gray-400 text-lg sm:text-xl md:text-2xl max-w-2xl mx-auto mt-8 font-bold tracking-tight leading-relaxed">
            Partner with <span className="text-orange-500">IndiaNext</span> — Mumbai&apos;s most ambitious 24-hour national hackathon at K.E.S. Shroff College.
          </motion.p>

          <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row items-center justify-center gap-4 mt-12">
            <a
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {STATS.map((stat, i) => (
              ...
              >
                ...
                <p className="text-2xl sm:text-3xl font-black tracking-tighter text-white">{stat.value}</p>
                <p className="text-[9px] text-gray-500 font-mono tracking-[0.3em] font-black mt-1">{stat.label}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="relative z-10 py-20 md:py-32">
        <div className="max-w-6xl mx-auto px-4 md:px-6">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              { icon: <Eye size={32} />, title: "Brand Visibility", desc: "Your logo across 400+ developer screens, social media, venue banners, posters, and certificates. Direct exposure to India's next-gen tech talent.", color: "#FF6600" },
              { icon: <Users size={32} />, title: "Talent Pipeline", desc: "Access to a curated pool of elite developers, designers and innovators. Direct resume access and recruitment opportunities.", color: "#00CCFF" },
              { icon: <Rocket size={32} />, title: "Impact & CSR", desc: "Support grassroots innovation in India. Associate your brand with education, technology, and national development.", color: "#00FF55" },
            ].map((item, i) => (
              <motion.div
                key={i}
              ...
              >
                <div className="w-16 h-16 flex items-center justify-center border border-white/10 bg-white/5 rounded-sm mb-8 group-hover:scale-110 transition-transform" style={{ color: item.color }}>
                  {item.icon}
                </div>
                <h3 className="text-2xl font-black uppercase tracking-tight italic mb-4">{item.title}</h3>
                <p className="text-gray-400 text-sm leading-relaxed font-bold tracking-tight">{item.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
            ...
            {TIERS.map((tier, i) => (
              ...
              >
                ...
                <div className="p-6 md:p-8 flex flex-col flex-1">
                  <p className="font-mono text-[9px] tracking-[0.4em] font-black uppercase mb-2" style={{ color: tier.color }}>{tier.name}</p>
                  <p className="text-3xl md:text-4xl font-black tracking-tighter mb-6">{tier.price}</p>

                  <div className="flex flex-col gap-3 flex-1">
                    {tier.perks.map((perk, j) => (
                      <div key={j} className="flex items-start gap-3">
                        <div className="w-1.5 h-1.5 rounded-full mt-1.5 shrink-0" style={{ backgroundColor: tier.color }} />
                        <span className="text-gray-400 text-xs font-bold tracking-tight leading-relaxed">{perk}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <a
                    ...
                    onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = tier.color; e.currentTarget.style.color = "#000"; }}
        ...
        <div className="max-w-5xl mx-auto px-4 md:px-6">
          ...
          >
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/5 bg-white/[0.02]">
              <span className="text-[10px] font-mono text-gray-500 tracking-widest font-black uppercase">SPONSOR_BROCHURE_V1.PDF</span>
              <a
                href="/Sponsor Brochures (A5).pdf"
                download
      ...
      <section className="relative z-10 py-20 md:py-32 bg-[#020202] border-t border-white/5">
        <div className="max-w-4xl mx-auto px-4 md:px-6 text-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer}>
            <motion.h2 variants={fadeInUp} className="text-4xl sm:text-6xl md:text-8xl font-black uppercase tracking-tighter italic leading-none mb-8">
              Let&apos;s Build <span className="text-orange-500">Together</span>
            </motion.h2>
            <motion.p variants={fadeInUp} className="text-gray-400 text-lg md:text-xl mb-12 font-bold tracking-tight max-w-2xl mx-auto">
              Ready to put your brand in front of India&apos;s sharpest student developers? Let&apos;s talk.
            </motion.p>

            <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-16">
              <a href="mailto:hackathon@kessc.edu.in?subject=Sponsorship%20Inquiry%20-%20IndiaNext%202026" className="px-8 py-4 bg-orange-500 text-black font-black text-sm tracking-widest uppercase rounded-sm hover:bg-orange-400 transition-all flex items-center gap-3 shadow-[0_0_30px_rgba(249,115,22,0.3)]">
                <Mail size={18} />
                HACKATHON@KESSC.EDU.IN
              </a>
              <a href="tel:+917506854879" className="px-8 py-4 border border-white/10 text-gray-300 font-black text-sm tracking-widest uppercase rounded-sm hover:text-white hover:border-white/30 transition-all flex items-center gap-3">
                <Phone size={18} />
                +91 75068 54879
              </a>
            </motion.div>
            ...
            <motion.div variants={fadeInUp} className="flex items-center justify-center gap-4">
              <Link href="/" className="text-gray-500 hover:text-white transition-colors font-mono text-xs tracking-widest font-bold uppercase">
                ← BACK TO HOME
              </Link>
            </motion.div>
          </motion.div>
