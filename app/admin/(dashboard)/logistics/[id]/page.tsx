...
export default function LogisticsTeamDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = trpc.logistics.getTeamById.useQuery({ teamId: id }, { refetchInterval: POLL_INTERVAL });
  ...
  const handleNotesUpdate = useCallback(async () => {
    try {
      await markTeamAttendance.mutateAsync({
        teamId: id,
        attendance: (currentTeam?.attendance || 'NOT_MARKED') as
          | 'PRESENT'
          | 'ABSENT'
          | 'NOT_MARKED'
          | 'PARTIAL',
    ...
    return (
      <div className="text-center py-12">
        <p className="text-xs font-mono text-gray-600 tracking-widest">
          TEAM NOT FOUND OR NOT APPROVED
        </p>
        <button
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span
              className={`text-[9px] font-mono font-bold px-1.5 py-0.5 rounded ${trackStyles[currentTeam.track] || ''}`}
            >
              {trackLabels[currentTeam.track] || currentTeam.track}
            </span>
          </div>
          <p className="text-[10px] font-mono text-gray-500 mt-1">
            {currentTeam.college || 'No college'} • {presentCount}/{currentTeam.members.length}{' '}
            members present
          </p>
        </div>
      ...
      <div className="bg-[#0A0A0A] rounded-lg border border-white/[0.06] p-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <span
            className={`inline-flex items-center gap-1 text-[10px] font-mono font-bold px-2.5 py-1 rounded border ${badge.style}`}
          >
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <button
                      ...
                      disabled={
                        markMemberAttendance.isPending || (member.checkedInAt !== null && !isAdmin)
                      }
                      title={
                        member.checkedInAt !== null && !isAdmin
                          ? 'Attendance locked — only Admin can change'
                          : undefined
                      }
