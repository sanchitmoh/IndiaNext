...
const glitchVariants = {
  animate: {
    x: [0, -2, 2, -2, 2, 0],
    y: [0, 1, -1, 1, -1, 0],
    filter: ['none', 'hue-rotate(90deg) opacity(0.8)', 'hue-rotate(-90deg) opacity(0.8)', 'none'],
    transition: {
      duration: 0.2,
      repeat: Infinity,
      repeatDelay: 5,
      ...
      {showArrow && (
        ...
        >
          <span className="animate-bounce-slow">
            <ChevronDown
              size={40}
              className="text-orange-400 drop-shadow-lg group-active:scale-90 transition-transform"
            />
          </span>
          <span className="text-xs text-orange-400 font-mono mt-1 tracking-widest bg-black/70 px-2 py-0.5 rounded">
            Scroll Down
          </span>
        </button>
      )}
      ...
      >
        <div className="flex flex-col gap-2 font-mono text-[8px] text-cyan-700 uppercase font-black tracking-widest">
          <div className="flex items-center gap-2">
            <div className="w-1 h-1 bg-cyan-700" /> LATENCY: 24MS
          </div>
          <div className="flex items-center gap-2">
            <div className="w-1 h-1 bg-cyan-700" /> PACKET_STATUS: NOMINAL
          </div>
          <div className="flex items-center gap-2">
            <div className="w-1 h-1 bg-cyan-700" /> ENCRYPTION: SHA-256
          </div>
        </div>
      </motion.div>
    </section>
      ...
      <div className="grid lg:grid-cols-2 gap-12 md:gap-24 items-center">
        <motion.div initial="hidden" animate="visible" variants={fadeInUp}>
          ...
          <h2 className="text-4xl sm:text-6xl md:text-[5.5rem] font-black mb-10 tracking-tighter uppercase leading-[0.85]">
            The Code That{' '}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-white to-blue-600 shadow-glow">
              Survives Tomorrow
            </span>
          </h2>
          <p className="text-gray-400 leading-relaxed text-lg sm:text-2xl mb-10 font-bold tracking-tight">
            We aren&apos;t just hacking for a day; we are building for the decade.{' '}
            <span className="text-orange-500">IndiaNext</span> is a National-Level Innovation
            Challenge empowering 400+ developers to build solutions for Bharat 2.0.
          </p>
          <div className="p-8 border-l-4 border-orange-600 bg-white/5 rounded-sm backdrop-blur-sm">
            ...
            <p className="text-gray-400 text-[10px] font-mono mt-2 tracking-widest uppercase">
              NAAC &apos;A&apos; GRADE | QS I-GAUGE GOLD RATING | MUMBAI, MH
            </p>
          </div>
        </motion.div>
  ...
  <section id="bounty" className="py-20 md:py-40 relative z-10 bg-black overflow-hidden">
    <div className="max-w-7xl mx-auto px-4 md:px-6">
        ...
        >
          ...
          <div className="space-y-8">
            {[
              ...
              <div
                key={idx}
                className="flex justify-between items-center p-4 sm:p-6 border border-white/10 bg-black group-hover:border-orange-500/30 transition-colors"
              >
                <span className="font-mono text-[10px] sm:text-xs text-gray-400 tracking-[0.15em] sm:tracking-[0.3em] font-black">
                  {p.label}
                </span>
                <span
                  className={`text-2xl sm:text-3xl md:text-4xl font-black ${p.color} font-mono tracking-tighter`}
                >
                  {p.prize}
                </span>
              </div>
        ...
        >
          ...
          <div className="space-y-8">
            {[
              ...
              <div
                key={idx}
                className="flex justify-between items-center p-4 sm:p-6 border border-white/10 bg-black group-hover:border-green-500/30 transition-colors"
              >
                <span className="font-mono text-[10px] sm:text-xs text-gray-400 tracking-[0.15em] sm:tracking-[0.3em] font-black">
                  {p.label}
                </span>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
        {['Incubation Support', 'Internship Referrals', 'Cloud Credits', 'Elite SWAG'].map(
          (perk, i) => (
            <motion.div
              key={i}
              whileHover={{ y: -5 }}
            ...
            >
              <span className="text-xs font-mono text-gray-400 tracking-[0.4em] uppercase font-black group-hover:text-cyan-400 transition-colors italic">
                {perk}
              </span>
            </motion.div>
          )
...
const TimelineSection = () => {
  ...
  const timelineEvents = [
    ...
    // Day 2
    {
      id: 8,
      event: 'Breakfast',
      timestamp: new Date('2026-03-17T07:00:00'),
      date: 'MAR 17, 2026',
      day: 'DAY 2',
      time: '07:00 – 08:00 AM',
      desc: 'MORNING FUEL',
    ...
    {
      id: 9,
      event: 'Mentorship Round 2',
      timestamp: new Date('2026-03-17T08:00:00'),
      date: 'MAR 17, 2026',
      day: 'DAY 2',
      time: '08:00 – 10:00 AM',
      desc: 'FINAL STRATEGY SYNC',
      icon: <Users />,
    },
    {
      id: 10,
      event: 'Development End',
      timestamp: new Date('2026-03-17T11:00:00'),
      date: 'MAR 17, 2026',
      day: 'DAY 2',
      time: '11:00 AM',
      desc: 'CODE FREEZE',
    ...
    },
    {
      ...
      timestamp: new Date('2026-03-17T13:00:00'),
      date: 'MAR 17, 2026',
      day: 'DAY 2',
      time: '01:00 PM onwards',
      desc: 'VICTORY PROTOCOL',
      icon: <Trophy />,
    },
  ];
    ...
    >
      <div className="max-w-5xl mx-auto px-4 md:px-6 relative">
        <div className="mb-16 md:mb-32 flex flex-col items-center">
          <p className="text-orange-500 font-mono text-[10px] tracking-[0.5em] sm:tracking-[1em] mb-4 uppercase font-black italic">
            {'// MISSION_CHRONOLOGY'}
          </p>
            ...
            {timelineEvents.map((item, i) => {
              ...
              return (
                ...
                >
                  {/* Center Point */}
                  <div
                    className={`absolute left-10 md:left-1/2 -translate-x-1/2 w-4 h-4 rounded-full bg-[#020202] border-2 ${isPast ? 'border-gray-800' : 'border-orange-500'} z-30 flex items-center justify-center`}
                  >
                    <div
                      className={`w-1.5 h-1.5 rounded-full ${isPast ? 'bg-gray-800' : 'bg-orange-500 animate-pulse'}`}
                    />
                  </div>
                  ...
                  <div className="flex-1 pl-20 md:pl-0">
                    <div
                      className={`flex flex-col ${i % 2 === 0 ? 'md:items-end md:text-right' : 'md:items-start md:text-left'}`}
                    >
                      <div className="flex items-center gap-4 mb-3">
                        {i % 2 !== 0 && (
                          <div
                            className={`w-10 h-10 rounded-sm flex items-center justify-center border ${isPast ? 'border-white/5 text-gray-800' : 'border-orange-500/50 text-orange-500'} bg-white/[0.02]`}
                          >
                        ...
                        <div className="space-y-0.5">
                          <span
                            className={`block font-mono text-[7px] sm:text-[9px] font-black tracking-wider sm:tracking-widest uppercase ${isPast ? 'text-gray-600' : 'text-orange-500'}`}
                          >
                            {item.day} • {item.date} • {item.time}
                          </span>
                          <h3
                            className={`text-xl sm:text-3xl font-black uppercase tracking-tighter italic ${isPast ? 'text-gray-500' : 'text-white'}`}
                          >
                            {item.event}
                          </h3>
                        </div>
                  ...
                  <div className="hidden md:flex flex-1 flex-col justify-center">
                    <div className={`p-4 ${i % 2 === 0 ? 'text-left pl-12' : 'text-right pr-12'}`}>
                      <span
                        className={`text-5xl font-black font-mono italic transition-colors uppercase select-none ${isPast ? 'text-white/5' : 'text-white/20 opacity-100 group-hover:text-white/40'}`}
                      >
                        {item.time}
                      </span>
                    </div>
                  ...
                  <div className="md:hidden absolute top-[-24px] left-[60px]">
                    <span
                      className={`text-[11px] font-mono font-black tracking-widest ${isPast ? 'text-gray-500' : 'text-orange-500/80'}`}
                    >
                      {item.time}
                    </span>
                  </div>
  ...
  <section className="py-20 md:py-40 relative z-10 bg-black">
    <div className="max-w-5xl mx-auto px-4 md:px-6 text-left">
      <div className="mb-12 md:mb-24">
        ...
        <p className="text-cyan-500 font-mono text-[10px] tracking-[0.5em] uppercase font-black">
          SYSTEM_QUERY_HANDLING
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-8">
        {[
          {
            q: 'Registration Fees?',
            a: 'Negative. IndiaNext is mission-critical and free for all elite teams selected.',
          },
          {
            q: 'Team Structure?',
            a: '2 to 4 operatives per squad. Inter-college alliances are authorized.',
          },
          {
            q: 'Pre-built Code?',
            a: 'Unauthorized. All systems must be engineered on-site. Timestamp audit in effect.',
          },
          {
            q: 'Equipment Loadout?',
            a: 'Bring your own hardware (Laptops, Chargers). Power grids and Wi-Fi uplink provided.',
          },
        ].map((faq, i) => (
          <motion.div
            key={i}
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10 md:gap-16 items-start mb-16 md:mb-24 text-left">
        <div className="col-span-1 sm:col-span-2">
          <div className="flex items-center gap-6 sm:gap-10 mb-8 md:mb-12">
            <Image
              ...
              width={80}
              height={80}
              className="object-contain md:w-[110px] md:h-[110px]"
            />
            <Image
              src="/KES 90 years logo in PNG format-01.png"
              alt="KES 90 Years"
              width={130}
              height={70}
              className="object-contain md:w-[180px] md:h-[100px]"
            />
          </div>
          ...
          <p className="text-gray-400 font-mono text-xs leading-relaxed uppercase tracking-[0.2em] font-black">
            Autonomous | NAAC &apos;A&apos; Grade (3.58 CGPA)
            <br />
            QS I-Gauge Gold | Best College Award (University of Mumbai)
            <br />
            Mumbai, MH 400067, IN
          </p>
        </div>
        <div>
          ...
          <div className="flex flex-col gap-6 text-xs font-black tracking-widest uppercase">
            <Link
              href="/rules"
              className="text-gray-400 hover:text-orange-500 transition-colors italic"
            >
              ./RULEBOOK_v1.0
            </Link>
            <Link
        ...
        <div>
          ...
          <div className="flex flex-col gap-5 text-xs font-black">
            <a
              ...
              className="text-cyan-400 hover:text-white transition-colors underline decoration-cyan-400/30"
            >
              HACKATHON@KESSC.EDU.IN
            </a>
            <p className="text-white tracking-widest italic">+91 75068 54879</p>
            <p className="text-gray-400 mt-4 text-xs border border-white/5 py-2 px-4 inline-block">
              @KES_SHROFF_COLLEGE
            </p>
          </div>
        </div>
      ...
      <div className="pt-10 md:pt-16 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6 md:gap-10">
        ...
        <p className="text-gray-400 text-[10px] sm:text-xs font-mono tracking-[0.3em] sm:tracking-[0.5em] font-black uppercase text-center">
          &copy; 2026 INDIANEXT // DECRYPTED_MISSION_DATA_SECURE
        </p>
      </div>
    </div>
  </footer>
...
const HoloCard = ({ title, subtitle, accent, icon, desc, tags }: HoloCardProps) => {
  ...
  return (
    ...
    >
      <motion.div className="pointer-events-none absolute -inset-px opacity-0 group-hover:opacity-100 transition duration-500">
        <div
        ...
        <div className="w-24 h-24 mb-12 flex items-center justify-center border border-white/10 bg-white/5 rounded-sm relative group-hover:scale-110 transition-transform">
          {React.isValidElement(icon) &&
            React.cloneElement(
              ...
              { size: 48, style: { color: accent } }
            )}
          <div className="absolute inset-0 bg-white/5 animate-pulse" />
        </div>

        <div
          className="mb-4 font-mono text-[10px] font-black tracking-[0.5em]"
          style={{ color: accent }}
        >
          {subtitle}
        </div>
        <h3 className="text-3xl sm:text-5xl md:text-6xl font-black mb-6 sm:mb-10 text-white uppercase tracking-tighter leading-[0.8] italic">
          {title}
        </h3>

        <p className="text-gray-400 text-base sm:text-xl mb-8 sm:mb-12 leading-tight flex-grow font-bold tracking-tight">
          {desc}
        </p>
...
const TechDetail = ({ icon, title, desc, accent = CYAN }: TechDetailProps) => (
  ...
  >
    <div
      className="absolute top-0 left-0 w-1 h-0 bg-orange-500 group-hover:h-full transition-all duration-300"
