...
function ConfirmBulkModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className={`bg-[#0d0d0d] border rounded-xl p-6 w-full max-w-sm mx-4 shadow-2xl ${isElim ? 'border-red-500/30 shadow-red-900/20' : 'border-emerald-500/30 shadow-emerald-900/20'}`}>
        <div className="flex items-center gap-3 mb-4">
          <div className={`w-10 h-10 rounded-lg flex items-center justify-center border ${isElim ? 'bg-red-500/10 border-red-500/20' : 'bg-emerald-500/10 border-emerald-500/20'}`}>
            {isElim ? <XCircle className="h-5 w-5 text-red-400" /> : <CheckCircle2 className="h-5 w-5 text-emerald-400" />}
          </div>
          <div>
            <p className="text-sm font-mono font-bold text-white">BULK {isElim ? 'ELIMINATE' : 'QUALIFY'}</p>
        ...
        <div className="flex gap-2">
          <button onClick={onCancel} className="flex-1 py-2 text-[11px] font-mono font-bold text-gray-400 bg-white/[0.03] border border-white/[0.08] rounded-lg hover:bg-white/[0.06] transition-all">
            CANCEL
          </button>
          <button
            onClick={onConfirm}
          ...
          >
            {isPending ? <Loader2 className="h-3 w-3 animate-spin" /> : isElim ? <XCircle className="h-3 w-3" /> : <CheckCircle2 className="h-3 w-3" />}
            CONFIRM
          </button>
        </div>
      </div>
...
function TeamRoundCard({
  ...
  const statusConfig: Record<RoundStatus, { label: string; color: string; icon: typeof CheckCircle2 }> = {
    PENDING: { label: 'PENDING', color: 'text-gray-400 bg-gray-500/10 border-gray-500/20', icon: Clock },
    QUALIFIED: { label: 'QUALIFIED', color: 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20', icon: CheckCircle2 },
    ELIMINATED: { label: 'ELIMINATED', color: 'text-red-400 bg-red-500/10 border-red-500/20', icon: XCircle },
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-2 mb-3">
        <button onClick={() => onToggleSelect(team.id)} className="mt-0.5 shrink-0 text-gray-500 hover:text-violet-400 transition-all">
          {selected ? <CheckSquare className="h-4 w-4 text-violet-400" /> : <Square className="h-4 w-4" />}
        </button>
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            <p className="text-sm font-mono font-bold text-white truncate">{team.name}</p>
            <span className="text-[9px] font-mono font-bold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-1.5 py-0.5 rounded">{team.shortCode}</span>
          </div>
        ...
        <span className={`inline-flex items-center gap-1 text-[9px] font-mono font-bold px-2 py-0.5 rounded border shrink-0 ${cfg.color}`}>
      ...
      <div className="mb-3">
        <div className="flex items-center justify-between mb-1">
          ...
          <span className="text-[10px] font-mono font-bold text-white">{team.score !== null ? team.score.toFixed(1) : '—'}</span>
        </div>
        <div className="h-1.5 bg-white/[0.04] rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-emerald-500 to-emerald-400 rounded-full transition-all duration-500" style={{ width: `${scorePercent}%` }} />
        </div>
      </div>
      ...
      {judgeName && actionAt && (
        <p className="text-[9px] font-mono text-gray-600 mb-2">
          By <span className="text-gray-400 font-bold">{judgeName}</span> • {new Date(actionAt).toLocaleString('en-IN', { dateStyle: 'short', timeStyle: 'short' })}
        </p>
      )}

      {/* Action buttons */}
      <div className="flex items-center gap-2">
        <Link href={`/admin/teams/${team.id}`} className="p-1.5 text-gray-500 hover:text-emerald-400 hover:bg-emerald-500/10 rounded transition-all" title="View team">
          <ChevronRight className="h-3.5 w-3.5" />
        </Link>
        {currentStatus !== 'ELIMINATED' && (
          <button onClick={() => onAction(team.id, 'ELIMINATED')} disabled={isPending}
        ...
        {currentStatus !== 'QUALIFIED' && (
          <button onClick={() => onAction(team.id, 'QUALIFIED')} disabled={isPending}
        ...
        {currentStatus !== 'PENDING' && (
          <button onClick={() => onAction(team.id, 'PENDING')} disabled={isPending} title="Reset to pending"
...
export function EliminationRoundPanel() {
  ...
  const [statusFilter, setStatusFilter] = useState<'all' | 'PENDING' | 'QUALIFIED' | 'ELIMINATED'>('all');
  ...
  const filteredTeams = useMemo(() => {
    ...
    return (teams as RoundTeam[]).filter(
      t => t.name.toLowerCase().includes(q) || t.shortCode.toLowerCase().includes(q) || (t.college && t.college.toLowerCase().includes(q))
    );
  }, [teams, search]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#0A0A0A] border border-white/[0.06] rounded-xl p-4">
        <div>
          <p className="text-xs font-mono font-bold text-gray-400 tracking-widest mb-1">ELIMINATION ROUNDS</p>
          <p className="text-[10px] font-mono text-gray-600">Qualify or eliminate teams per round</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          {(['1', '2'] as Round[]).map(r => (
            <button key={r} onClick={() => { setActiveRound(r); setSelectedIds(new Set()); }}
              className={`px-4 py-2 text-[11px] font-mono font-bold rounded-lg border transition-all flex items-center gap-1.5 ${
                activeRound === r
                  ? r === '1' ? 'bg-violet-500/20 text-violet-300 border-violet-500/40' : 'bg-orange-500/20 text-orange-300 border-orange-500/40'
          ...
          {canAdvance && activeRound === '1' && (
            <button onClick={() => { if (confirm(`Advance ${stats.qualified} qualified teams to Round 2?`)) advance.mutate(); }}
              ...
              {advance.isPending ? <Loader2 className="h-3 w-3 animate-spin" /> : <ArrowRight className="h-3 w-3" />}
              ADVANCE TO R2
            </button>
          )}
        </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          <div key={label} className="bg-[#0A0A0A] border border-white/[0.06] rounded-lg p-3 text-center">
      ...
      {stats.total > 0 && (
        <div className="bg-[#0A0A0A] border border-white/[0.06] rounded-lg p-3">
          ...
          <div className="h-2 bg-white/[0.04] rounded-full overflow-hidden flex">
            <div className="h-full bg-emerald-500 transition-all duration-500" style={{ width: `${(stats.qualified / stats.total) * 100}%` }} />
            <div className="h-full bg-red-500/80 transition-all duration-500" style={{ width: `${(stats.eliminated / stats.total) * 100}%` }} />
          </div>
      ...
      <div className="flex flex-col gap-2">
        {/* Select-All row */}
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-[9px] font-mono text-gray-600 uppercase tracking-wide">SELECT:</span>
          {[
            { label: 'ALL', track: 'all' as const, color: 'text-gray-300 border-white/[0.08]' },
            { label: '💡 IDEA SPRINT', track: 'IDEA_SPRINT' as const, color: 'text-cyan-400 border-cyan-500/20' },
            { label: '⚡ BUILD STORM', track: 'BUILD_STORM' as const, color: 'text-orange-400 border-orange-500/20' },
          ].map(({ label, track, color }) => (
            <button key={track} onClick={() => selectByTrack(track)}
          ...
          {numSelected > 0 && (
            <button onClick={clearSelection} className="px-2 py-1 text-[9px] font-mono text-gray-600 hover:text-gray-400 border border-white/[0.06] rounded transition-all">
              CLEAR ({numSelected})
            </button>
          )}
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Search team, code, college..." value={search}
      ...
      {numSelected > 0 && (
        <div className="flex items-center gap-3 bg-violet-500/10 border border-violet-500/20 rounded-lg px-4 py-2.5">
          ...
          <button onClick={clearSelection} className="p-1.5 text-gray-600 hover:text-gray-400 rounded transition-all">
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-48 bg-white/[0.02] border border-white/[0.04] rounded-xl animate-pulse" />
          ))}
        </div>
        ...
        <div className="text-center py-16 border border-dashed border-white/[0.06] rounded-xl">
          ...
          <p className="text-xs font-mono text-gray-600">
            {activeRound === '2' ? 'No teams qualified from Round 1 yet' : 'No teams match the current filters'}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      {confirmElimTeam && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="bg-[#0d0d0d] border border-red-500/30 rounded-xl p-6 w-full max-w-sm mx-4 shadow-2xl shadow-red-900/20">
            ...
            <p className="text-sm font-mono font-bold text-red-400 mb-1">{confirmElimTeam.name} <span className="text-[10px] text-gray-600">({confirmElimTeam.shortCode})</span></p>
            <p className="text-[11px] font-mono text-gray-600 mb-5">This team will be eliminated from Round {activeRound}. You can undo via PENDING reset.</p>
            <div className="flex gap-2">
              <button onClick={() => setConfirmElimId(null)} className="flex-1 py-2 text-[11px] font-mono font-bold text-gray-400 bg-white/[0.03] border border-white/[0.08] rounded-lg hover:bg-white/[0.06] transition-all">CANCEL</button>
              <button onClick={() => { setStatus.mutate({ teamId: confirmElimTeam.id, round: activeRound, status: 'ELIMINATED' }); setConfirmElimId(null); }}
                ...
                {setStatus.isPending ? <Loader2 className="h-3 w-3 animate-spin" /> : <XCircle className="h-3 w-3" />}
                ELIMINATE
              </button>
            </div>
          </div>
