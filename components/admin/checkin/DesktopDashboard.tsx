      ...
      <div className="fixed inset-0 bg-black flex flex-col items-center justify-center p-12 font-mono">
        <div className="w-full max-w-xl space-y-12">
          <div className="text-center space-y-4">
            <div className="inline-block px-3 py-1 rounded-full bg-orange-500/10 border border-orange-500/20 text-orange-500 text-[10px] font-bold tracking-[0.2em] mb-4">SYSTEM_INITIALIZATION</div>
            <h1 className="text-4xl font-black text-white tracking-widest uppercase mb-2">Select_Command_Station</h1>
            <p className="text-zinc-500 text-xs tracking-wider">BIND THIS TERMINAL TO A PHYSICAL CHECK-IN DESK FOR REAL-TIME SYNC</p>
          </div>
          <div className="grid grid-cols-4 gap-6">
            {['A', 'B', 'C', 'D'].map(id => (
              ...
              >
                <div className="text-5xl font-black text-zinc-700 group-hover:text-orange-500 transition-colors">{id}</div>
                <div className="text-[10px] text-zinc-800 group-hover:text-orange-500/50 mt-2 font-bold uppercase tracking-widest">Station</div>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="p-6 border-b border-white/5 space-y-6">
          <div className="flex justify-between items-center">
            <div className="flex flex-col">
              <h2 className="text-[9px] font-bold tracking-[0.4em] text-zinc-500 uppercase leading-none mb-1">Station_{deskId}</h2>
              <span className="text-[8px] text-zinc-600 font-bold uppercase tracking-widest">Operator Terminal</span>
            </div>
            <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-full ${isConnected ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-500 shadow-[0_0_10px_rgba(239,68,68,0.2)]'}`}>
              ...
              <span className="text-[8px] font-bold uppercase tracking-widest">{isConnected ? 'Link_Active' : 'Offline'}</span>
            </div>
          </div>
          ...
          <div className="bg-white/5 rounded-xl p-5 border border-white/5 relative overflow-hidden group">
            ...
            <p className="text-[8px] text-zinc-500 mb-1 font-bold tracking-widest">OCCUPANCY_METRICS</p>
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-black text-orange-500 tracking-tighter">{stats?.checkedIn || 0}</span>
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <p className={`text-[10px] font-bold ${activeTeam?.id === team.id ? 'text-orange-400' : 'text-zinc-300'} truncate group-hover:text-white transition-colors`}>{team.name}</p>
                        <p className="text-[8px] text-zinc-600 mt-1 font-mono tracking-widest">{team.shortCode}</p>
                      </div>
          ...
          {activeTeam ? (
            ...
            >
              ...
              <div className="p-10 border-b border-white/5 bg-gradient-to-br from-white/[0.02] to-transparent flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-3 mb-4">
                    <div className={`px-2 py-0.5 rounded text-[9px] font-black tracking-[0.2em] uppercase border ${activeTeam.track === 'IDEA_SPRINT' ? 'bg-cyan-500/10 text-cyan-400 border-cyan-500/20' : 'bg-orange-500/10 text-orange-400 border-orange-500/20'}`}>
                      {activeTeam.track.replace('_', ' ')}
                    </div>
                    <div className="h-1 w-1 rounded-full bg-zinc-800" />
                    <span className="text-zinc-500 text-[10px] font-bold tracking-widest">{activeTeam.shortCode}</span>
                  </div>
                  <h1 className="text-4xl font-black tracking-tight text-white mb-3 uppercase">{activeTeam.name}</h1>
                  <div className="flex items-center gap-2 text-zinc-500">
                    <MapPin className="h-3.5 w-3.5" />
                    <p className="text-[11px] font-bold tracking-wider">{activeTeam.college || 'Universal Academy of Tech'}</p>
                  </div>
                </div>
                <div className="text-right flex flex-col items-end">
                  <span className="text-[8px] font-black text-zinc-600 tracking-[0.4em] mb-2 uppercase">Assignment_Ref</span>
                  <div className="w-20 h-20 rounded-2xl bg-white/[0.03] border border-white/5 flex items-center justify-center relative overflow-hidden group">
                    ...
                    <span className="text-5xl font-black text-white relative z-10">{selectedDesk || '--'}</span>
                  </div>
                </div>
              </div>
                  ...
                  <div className="space-y-4">
                    {activeTeam.members?.map((m: any, i: number) => (
                      <div key={i} className="flex items-center gap-4 group">
                        ...
                        <div className="min-w-0">
                          <p className="text-xs text-white font-bold group-hover:text-orange-400 transition-colors">{m.user.name}</p>
                ...
                <div className="space-y-8">
                  ...
                  <div className="p-5 bg-orange-500/[0.03] border border-orange-500/10 rounded-xl flex items-start gap-4">
                    ...
                    <div className="space-y-1">
                      <p className="text-[10px] text-zinc-300 font-bold leading-none uppercase tracking-widest">Protocol Check</p>
              ...
              <div className="p-10 bg-[#0A0A0A] border-t border-white/5 flex gap-4">
                ...
                >
                  {confirmMutation.isPending ? <RotateCcw className="h-4 w-4 animate-spin" /> : <UserCheck className="h-5 w-5" />}
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-col items-center gap-2">
                  ...
                  <div className="flex items-center gap-2 mt-4 px-3 py-1 bg-white/[0.02] border border-white/5 rounded-full">
                    ...
                    <span className="text-[8px] font-bold text-zinc-600 uppercase tracking-widest">Listening on station {deskId}</span>
                  </div>
                </div>
              </div>
            </motion.div>
