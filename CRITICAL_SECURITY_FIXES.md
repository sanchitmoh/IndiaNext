...
```typescript
const isDemoEmail =
  email.includes('demo.idea@example.com') || email.includes('demo.build@example.com');
```
...
```typescript
// On session creation
const fingerprint = generateSessionFingerprint(getUserAgent(req), getClientIp(req));
...
const currentFingerprint = generateSessionFingerprint(getUserAgent(req), getClientIp(req));
...
describe('getTeamById', () => {
  it('should not return soft-deleted teams', async () => {
    ...
    await expect(caller.getTeamById({ id: team.id })).rejects.toThrow('NOT_FOUND');
  });
});
```
